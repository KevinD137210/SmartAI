...
import { Plus, Printer, Eye, Trash2, Edit2, ArrowLeft, Download, FileText, Users, Mail, Sparkles, Loader2, ExternalLink, Paperclip, X, Search, AlertCircle, ArrowRightCircle, Briefcase, Archive } from 'lucide-react';
...
export const Invoices: React.FC<InvoicesProps> = ({ invoices, clients = [], projects = [], onSaveInvoice, onDeleteInvoice, onUpdateStatus, onArchiveClient }) => {
  ...
  // Handle incoming navigation state for pre-filling invoice
  useEffect(() => {
    if (location.state && location.state.createNew) {
        ...
        // Clear state to avoid reopening on refresh (optional, usually router handles it but explicit is safer)
  ...
  const handleArchiveClient = (inv: Invoice) => {
      // Find client by name/email approximation if ID not stored directly on invoice (Invoice structure improvement: store clientId)
  ...
  const handlePrint = () => {
    ...
    if (doc) {
            ...
            <body>
                ...
                <script>
                    window.onload = function() {
                        setTimeout(function() {
                            window.print();
                            // Notify parent to remove iframe (optional, but clean)
                            window.parent.postMessage('printComplete', '*');
                        }, 500); // Allow tailwind to load
                    }
                </script>
            </body>
            </html>
        ...
        // Cleanup listener
        const cleanup = () => {
            // Check if iframe is still a child of body before removing
                        ...
                        <div key={item.id} className="flex gap-4 items-start">
                            <div className="flex-1">
                                <input
                                    ...
                                    className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white"
                                />
                            </div>
                            <div className="w-24">
                                <input
                                    ...
                                    className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white text-right"
                                />
                            </div>
                            <div className="w-32">
                                <input
      ...
      return (
          <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn pb-20">
            ...
            <div className="bg-white text-slate-900 shadow-xl rounded-none md:rounded-lg p-12 min-h-[1000px]" id="print-area">
                {/* Print layout always light mode for paper saving and standard look */}
      ...
      <div className="grid grid-cols-1 gap-4">
        {invoices.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800">
                ...
                <p className="text-slate-400 dark:text-slate-500 font-medium">{t('inv.noDocs')}</p>
            </div>
        ) : (
            invoices.map(inv => (
